...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://build-a-blog:buildablogpass@localhost:8889/build-a-blog'
...
def blog():
    ...
    if request.method == 'POST': 
        ...
        if len(blog_title) <= 0 or len(blog_post) <=0: 
    ...
    if blog_id:
        ...
        return render_template('single_blog.html', 
                                ...
                                single_post=blog_obj.blog_post) 
...
def new_post():
    ...
    if title_error == None or body_error == None: 
        title_error = ''
        body_error = ''
...
def single_blog():
    blog_object = Blog.query.get(int(request.args.get('id'))) 
